...
3. 新建环境(可一键随机生成指纹),填写代理、标签和备注后启动。可从代理库复用端点,也可以复制环境或保存为模板。代理默认按“系统代理/V2Ray → 指定出口代理”串联,也可关闭系统代理前置。
...
> 每个环境可设置出口画像变化策略(提醒、阻止、自动更新)和代理 IP 不可用策略(询问、阻止、使用系统代理)。使用系统代理回退只作用于本次启动,不会覆盖环境保存的指定代理。启动一个或多个环境前会根据可用内存给出预检提示。
...
- [x] 阶段四首批:profile manifest v2、出口 GeoIP 多源回退、IP 到时区/语言联动、启动前一致性与内存检查、系统代理串联
